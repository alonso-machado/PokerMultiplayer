...
function scoreHand(cards) {
    ...
    if (counts[0][0] === 4)
        return { rank: 7, tiebreakers: sortedCounts(counts), name: 'Four of a Kind' };
    if (counts[0][0] === 3 && counts[1][0] === 2)
        return { rank: 6, tiebreakers: sortedCounts(counts), name: 'Full House' };
    if (isFlush)
        return { rank: 5, tiebreakers: values, name: 'Flush' };
    if (isStraight)
        return { rank: 4, tiebreakers: [strHigh], name: 'Straight' };
    if (counts[0][0] === 3)
        return { rank: 3, tiebreakers: sortedCounts(counts), name: 'Three of a Kind' };
    if (counts[0][0] === 2 && counts[1][0] === 2)
        return { rank: 2, tiebreakers: sortedCounts(counts), name: 'Two Pair' };
    if (counts[0][0] === 2)
        return { rank: 1, tiebreakers: sortedCounts(counts), name: 'Pair' };
...
function straightHighCard(values) {
    ...
    if (unique[0] === 14) {
        ...
        if (low.length >= 4 && low[0] - low[3] === 3 && low[3] === 2)
            return 5;
    }
    return null;
}
