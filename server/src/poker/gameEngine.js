...
export class PokerGame {
    ...
    applyAction(playerId, action, amount = 0) {
        ...
        switch (action) {
            ...
            case 'raise': {
                if (this._noReraiseIds.has(player.id))
                    return false;
                const raiseTotal = Math.min(amount, player.chips + player.bet);
                if (raiseTotal < this._currentBet + this._minRaise)
                    return false;
            ...
            case 'all-in': {
                ...
                if (player.bet > this._currentBet) {
                    ...
                    if (raiseBy >= this._minRaise) {
                        ...
                        for (const id of this._actedThisStreet)
                            this._noReraiseIds.add(id);
                    }
    ...
    validActions(p) {
        ...
        if (toCall > 0 && p.chips >= toCall)
            actions.push('call');
        if (p.chips > toCall && !this._noReraiseIds.has(p.id))
            actions.push('raise');
        if (p.chips > 0)
            actions.push('all-in');
