...
export function adminRouter(getTournamentInfo, createTournament, startTournament, deleteTournament) {
    return async (req, url) => {
        ...
        if (path === '/api/admin/tournament' && method === 'POST') {
            ...
            if (!body.scheduledStart)
                return json({ error: 'Data obrigatória.' }, 400);
            const cfg = body.config;
            if (!cfg || cfg.bigBlind < 2 || cfg.smallBlind < 1)
                return json({ error: 'Blinds inválidos.' }, 400);
...
export function publicTournamentHandler(getTournamentInfo) {
