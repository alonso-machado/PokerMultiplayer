...
function buildBlindSchedule(base) {
    const levels = [];
...
export class Tournament {
    ...
    // Blind schedule
    blindSchedule;
    currentBlindLevelIdx = 0;
    blindLevelStart = 0; // Date.now() when current level started
    blindTimer = null;
    // Timers
    startTimer = null;
    rankingTimer = null;
    onTablesChanged;
    onFinished;
    constructor(id, cfg, onTablesChanged, onFinished) {
        this.id = id;
        this.name = cfg.name;
        ...
        if (ms > 0)
            this.startTimer = setTimeout(() => this.autoStart(), ms);
    }
    // ── Info ──────────────────────────────────────────────────────────────────
    info() {
    ...
    // ── Registration ──────────────────────────────────────────────────────────
    register(pid, name, send, token) {
        if (this.status !== 'registering')
            return false;
        const reg = { playerId: pid, name, token, send };
        this.byToken.set(token, reg);
        this.registrations.set(pid, reg);
    ...
    findByToken(token) { return this.byToken.get(token); }
    isRegistered(pid) { return this.registrations.has(pid); }
    updateSendFn(pid, send) {
        const reg = this.registrations.get(pid);
        if (!reg)
            return;
        reg.send = send;
        const t = this.byToken.get(reg.token);
        if (t)
            t.send = send;
    }
    // ── Start ─────────────────────────────────────────────────────────────────
    autoStart() {
        if (this.status !== 'registering' || this.registrations.size < 2)
            return;
        this.start();
    }
    start() {
        ...
        for (const reg of this.registrations.values()) {
            ...
            if (tp?.tableId) {
                ...
                if (room)
                    reg.send({ type: 'tournament_table_assigned', roomId: room.id, roomName: room.name, config: room.config });
            }
        }
    ...
    advanceBlinds() {
        ...
        if (this.currentBlindLevelIdx >= this.blindSchedule.length)
            return;
    ...
    distributeToTables() {
        ...
        for (let i = 0; i < count; i++) {
            const room = new Room(generateId(), `${this.name} — Mesa ${i + 1}`, this.name, this.config, { tournamentId: this.id, onPlayersEliminated: (eliminations) => this.onEliminated(eliminations) });
    ...
    onEliminated(eliminations) {
        ...
        for (const { playerId: pid } of ordered) {
            const tp = this.activePlayers.get(pid);
            if (!tp || tp.eliminated)
                continue;
            tp.eliminated = true;
            tp.chips = 0;
    ...
    checkRebalance() {
        ...
        if (largest.playerCount - smallest.playerCount >= 2 && !smallest.isFull) {
            ...
            if (!gp)
                return;
            const send = largest.getSendFn(gp.id);
            if (!send)
                return;
            const moved = largest.moveTournamentPlayer(gp.id);
            if (!moved)
                return;
            ...
            if (tp) {
                tp.tableId = smallest.id;
                tp.tableName = smallest.name;
            }
        }
    ...
    checkFinalTable() {
        ...
        if (remaining.length > FINAL_TABLE_THRESHOLD || this.status !== 'running')
            return;
        ...
        const finalRoom = new Room(generateId(), `${this.name} — Mesa Final`, this.name, this.config, { tournamentId: this.id, onPlayersEliminated: (eliminations) => this.onEliminated(eliminations) });
        this.tables.set(finalRoom.id, finalRoom);
        for (const tp of remaining) {
            const old = tp.tableId ? this.tables.get(tp.tableId) : null;
            const send = old?.getSendFn(tp.id);
            if (!send)
                continue;
            ...
            tp.tableId = finalRoom.id;
            tp.tableName = finalRoom.name;
    ...
    getRanking() {
        for (const [tableId, room] of this.tables) {
            for (const tp of this.activePlayers.values()) {
                if (tp.tableId === tableId && !tp.eliminated)
                    tp.chips = room.getPlayerChips(tp.id);
            }
        }
    ...
    getTableId(pid) { return this.activePlayers.get(pid)?.tableId ?? null; }
