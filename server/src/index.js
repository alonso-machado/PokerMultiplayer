...
function currentTrucoRoom(session) {
    return session.trucoRoomId ? trucoRooms.get(session.trucoRoomId) : undefined;
}
// ── Admin callbacks ───────────────────────────────────────────────────────────
const handleAdmin = adminRouter(() => activeTournament?.info() ?? null, (data) => {
    ...
    try {
        ...
        activeTournament = new Tournament(generateId(), { name: data.name.trim().slice(0, 40), scheduledStart, config: cfg }, (tRooms) => { for (const [id, r] of tRooms)
            rooms.set(id, r); broadcastRoomList(); }, () => {
            setTimeout(() => {
                if (!activeTournament)
                    return;
                for (const r of activeTournament.tableMap.values())
                    r.destroy();
                for (const id of activeTournament.tableMap.keys())
                    rooms.delete(id);
                broadcastRoomList();
            }, 60_000);
        });
    ...
    if (!activeTournament)
        return { ok: false, error: 'Nenhum torneio.' };
    if (activeTournament.status !== 'registering')
        return { ok: false, error: 'Já iniciado.' };
    ...
    fetch(req, server) {
        ...
        if (pathname === '/ws') {
            const ok = server.upgrade(req, {
                data: { playerId: '', name: 'Jogador', roomId: null, trucoRoomId: null, tournamentToken: null },
            });
            return ok ? undefined : new Response('Upgrade failed', { status: 400 });
        }
        if (pathname.startsWith('/api/admin/'))
            return handleAdmin(req, url);
        if (pathname === '/api/tournament')
            return handlePublicTournament(req, url);
        // ── Docs (dev only) ───────────────────────────────────────────────────────
        if (IS_DEV) {
    ...
    websocket: {
        ...
        async message(ws, raw) {
            ...
            switch (msg.type) {
                ...
                case 'list_rooms':
                    emit({ type: 'room_list', rooms: lobbyRoomList() });
                    break;
                case 'get_tournament':
                    emit({ type: 'tournament_info', tournament: activeTournament?.info() ?? null });
                    break;
                // ── Create lobby room ───────────────────────────────────────────────
                case 'create_room': {
                    if (lobbyRoomCount() >= MAX_LOBBY_ROOMS) {
                        emit({ type: 'room_error', message: `Limite de ${MAX_LOBBY_ROOMS} salas atingido.` });
                        break;
                    }
                ...
                case 'join_room': {
                    ...
                    if (!room) {
                        emit({ type: 'room_error', message: 'Sala não encontrada.' });
                        break;
                    }
                    if (room.isFull) {
                        emit({ type: 'room_error', message: 'Sala cheia.' });
                        break;
                    }
                    if (room.tournamentId) {
                        emit({ type: 'room_error', message: 'Mesa de torneio.' });
                        break;
                    }
                    if (session.roomId)
                        leaveRoom(ws);
                ...
                case 'rebuy': {
                    ...
                    if (room && !room.tournamentId)
                        room.handleRebuy(session.playerId);
                    break;
                }
                case 'rebuy_decline': {
                    const room = session.roomId ? rooms.get(session.roomId) : undefined;
                ...
                case 'set_away': {
                    const room = currentRoom(session);
                    if (room?.tournamentId)
                        room.setAway(session.playerId);
                    break;
                }
                case 'set_back': {
                    const room = currentRoom(session);
                    if (room?.tournamentId)
                        room.setBack(session.playerId);
                    break;
                }
                // ── Truco ────────────────────────────────────────────────────────────
                case 'truco_list_rooms':
                    emit({ type: 'truco_room_list', rooms: trucoRoomList() });
                    break;
                ...
                case 'truco_join_room': {
                    ...
                    if (!room) {
                        emit({ type: 'truco_room_error', message: 'Mesa não encontrada.' });
                        break;
                    }
                    if (room.isFull) {
                        emit({ type: 'truco_room_error', message: 'Mesa cheia.' });
                        break;
                    }
                    if (room.isStarted) {
                        emit({ type: 'truco_room_error', message: 'Partida em andamento.' });
                        break;
                    }
                    if (session.trucoRoomId)
                        leaveTrucoRoom(ws);
                    room.join(session.playerId, session.name, emit);
                    session.trucoRoomId = room.id;
                    setPersistentTrucoRoom(session.playerId, room.id);
                ...
                case 'register_tournament': {
                    if (!activeTournament) {
                        emit({ type: 'tournament_error', message: 'Nenhum torneio disponível.' });
                        break;
                    }
                    if (activeTournament.status !== 'registering') {
                        emit({ type: 'tournament_error', message: 'Inscrições encerradas.' });
                        break;
                    }
                    if (activeTournament.isRegistered(session.playerId))
                        break;
                ...
                case 'unregister_tournament': {
                    if (!activeTournament || activeTournament.status !== 'registering')
                        break;
...
function setPersistentRoom(pid, roomId) {
    const ps = playerSessions.get(pid);
    if (ps)
        ps.roomId = roomId;
}
function leaveTrucoRoom(ws) {
...
function setPersistentTrucoRoom(pid, trucoRoomId) {
    const ps = playerSessions.get(pid);
    if (ps)
        ps.trucoRoomId = trucoRoomId;
}
function setPersistentToken(pid, token) {
    const ps = playerSessions.get(pid);
    if (ps)
        ps.tournamentToken = token;
}
...
function broadcastRoomList() {
    server.publish('lobby', JSON.stringify({ type: 'room_list', rooms: lobbyRoomList() }));
}
function broadcastTrucoRoomList() {
    server.publish('lobby', JSON.stringify({ type: 'truco_room_list', rooms: trucoRoomList() }));
}
function broadcastTournamentInfo() {
    server.publish('lobby', JSON.stringify({ type: 'tournament_info', tournament: activeTournament?.info() ?? null }));
}
